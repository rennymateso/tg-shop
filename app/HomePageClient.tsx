...
function SearchIcon() {
  return <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round"><circle cx="11" cy="11" r="7" /><path d="M20 20L17 17" /></svg>;
}

function FilterIcon() {
  return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 6h16" /><path d="M7 12h10" /><path d="M10 18h4" /></svg>;
}

function ChevronDownIcon() {
  return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m6 9 6 6 6-6" /></svg>;
}
      ...
      <main className="min-h-screen bg-[#F5F5F5] px-3 pt-[76px] pb-32 text-black">
        <div className="mb-4 flex items-center justify-center">
          <button type="button" onClick={resetPage} className="text-center">
            <p className="text-[18px] font-semibold tracking-[0.13em] text-black">MONTREAUX</p>
            <p className="mt-0.5 text-[8px] uppercase tracking-[0.32em] text-gray-400">Fashion</p>
          </button>
        </div>
              ...
              {currentCategories.map((category) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className={`mt-1.5 truncate text-center text-[11px] ${isActive ? "font-medium text-black" : "text-[#525252]"}`}>
                      {category}
                    </p>
                  </button>
                );
        ...
        <div className="mt-3 overflow-hidden rounded-[20px] bg-white shadow-[0_12px_34px_rgba(15,23,42,0.06)]">
          <button type="button" onClick={() => router.push(banners[0].link)} className="relative block h-[176px] w-full overflow-hidden">
            <img src={banners[0].image} alt={banners[0].alt} className="h-full w-full object-cover" />
            ...
            <div className="absolute left-4 top-4 max-w-[190px] text-left">
              ...
              <h2 className="mt-1 text-[24px] font-semibold leading-[1.05] tracking-[-0.03em] text-white">Весна Лето 2026</h2>
              <span className="mt-4 inline-flex rounded-[10px] bg-white px-4 py-2 text-[12px] font-medium text-black">Смотреть</span>
            </div>
          </button>
        </div>
        ...
        <div className="mt-3 grid grid-cols-[1fr_1fr_auto] gap-2">
          <div className="relative" ref={brandMenuWrapRef}>
            <button type="button" onClick={() => setShowBrandMenu((prev) => !prev)} className="flex h-[42px] w-full items-center justify-between rounded-[13px] bg-white px-3 text-[12px] font-medium text-black shadow-[0_6px_18px_rgba(0,0,0,0.04)]">
            ...
            {showBrandMenu && (
              <div className="absolute left-0 top-[48px] z-50 max-h-72 w-full overflow-y-auto rounded-2xl bg-white p-1.5 shadow-[0_16px_40px_rgba(0,0,0,0.14)]">
                <button type="button" onClick={() => { setSelectedBrand("Все бренды"); setShowBrandMenu(false); }} className={`w-full rounded-xl px-3 py-2 text-left text-[13px] ${selectedBrand === "Все бренды" ? "bg-black text-white" : "text-gray-700"}`}>
                  Все бренды
                </button>

                {initialBrands.map((brand) => (
                  <button key={brand.id} type="button" onClick={() => { setSelectedBrand(brand.name); setShowBrandMenu(false); }} className={`w-full rounded-xl px-3 py-2 text-left text-[13px] ${selectedBrand === brand.name ? "bg-black text-white" : "text-gray-700"}`}>
                    {brand.name}
                  </button>
                ))}
              </div>
          ...
          <div className="relative" ref={sortMenuRef}>
            <button type="button" onClick={() => setShowSortMenu((prev) => !prev)} className="flex h-[42px] w-full items-center justify-between rounded-[13px] bg-white px-3 text-[12px] font-medium text-black shadow-[0_6px_18px_rgba(0,0,0,0.04)]">
            ...
            {showSortMenu && (
              <div className="absolute right-0 top-[48px] z-50 w-full rounded-2xl bg-white p-1.5 shadow-[0_10px_30px_rgba(0,0,0,0.10)]">
                {sortOptions.map((option) => (
                  <button key={option} type="button" onClick={() => { setSelectedSort(option); setShowSortMenu(false); }} className={`w-full rounded-xl px-3 py-2 text-left text-[13px] ${selectedSort === option ? "bg-black text-white" : "text-gray-700"}`}>
                    {option}
                  </button>
                ))}
              </div>
          ...
          <div className="relative" ref={availabilityMenuRef}>
            <button type="button" onClick={() => setShowAvailabilityMenu((prev) => !prev)} className="flex h-[42px] w-[44px] items-center justify-center rounded-[13px] bg-white text-black shadow-[0_6px_18px_rgba(0,0,0,0.04)]">
              <FilterIcon />
            </button>
            ...
            {showAvailabilityMenu && (
              <div className="absolute right-0 top-[48px] z-50 w-52 rounded-2xl bg-white p-1.5 shadow-[0_16px_40px_rgba(0,0,0,0.14)]">
                {["Все товары", "В наличии", "Из-за рубежа"].map((option) => (
                  <button key={option} type="button" onClick={() => { setSelectedAvailability(option); setShowAvailabilityMenu(false); }} className={`w-full rounded-xl px-3 py-2 text-left text-[13px] ${selectedAvailability === option ? "bg-black text-white" : "text-gray-700"}`}>
                    {option}
                  </button>
                ))}
              </div>
        ...
        {filteredProducts.length === 0 ? (
          <div className="mt-4 rounded-[22px] bg-white p-7 text-center shadow-[0_8px_28px_rgba(0,0,0,0.05)]">
            <p className="text-[16px] font-medium text-black">{selectedDepartment === "Женщинам" ? "Женский раздел пока в разработке" : "Ничего не найдено"}</p>
            <p className="mt-2 text-sm text-gray-400">{selectedDepartment === "Женщинам" ? "Скоро здесь появятся товары" : "Попробуйте изменить фильтры"}</p>
          </div>
        ) : (
          <div className="mt-4 grid grid-cols-2 gap-[8px]">
            {filteredProducts.map((p) => {
              ...
              return (
                <article key={p.id} onClick={() => router.push(`/product?id=${p.id}`)} onMouseEnter={() => router.prefetch(`/product?id=${p.id}`)} className="cursor-pointer overflow-hidden rounded-[14px] bg-white shadow-[0_8px_22px_rgba(0,0,0,0.07)]">
                  <div
